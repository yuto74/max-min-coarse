...
def coarsen_graph_by_clustering(g_orig: graph_tools.Graph, node_map: dict, args: argparse.Namespace, target_node_count: int):    
    """
...
def main():
    ...
    # ... 既存の引数（input, output, flows, embedding_dim, hidden_dim, bottleneck_penalty） ...
    ...
    # ★ alphaとtarget_graphを排他的な必須引数グループに変更
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('-a', '--alpha', type=float, help='粗視化率 (0 < alpha <= 1)')
    group.add_argument('-t', '--target_graph', type=str, help='目標ノード数を持つグラフのDOTファイルパス')
    ...
    if args.alpha:
        ...
        num_nodes = g_tools_orig.nvertices()
        target_node_count = int(math.ceil(args.alpha * num_nodes))
        print(f"粗視化率 {args.alpha} に基づき、目標ノード数を {target_node_count} に設定しました。")
    elif args.target_graph:
        print(f"目標グラフ {args.target_graph} を読み込み中...")
        try:
            g_target = graph_tools.Graph(directed=False)
            with open(args.target_graph) as f:
                lines = f.readlines()
            g_target.import_dot(lines)
            target_node_count = g_target.nvertices()
            print(f"目標グラフのノード数に基づき、目標ノード数を {target_node_count} に設定しました。")
        except FileNotFoundError:
            parser.error(f"指定された目標グラフファイルが見つかりません: {args.target_graph}")

    # ★coarsen_graph_by_clustering に target_node_count を渡す
    g_coarse = coarsen_graph_by_clustering(g_tools_orig, node_map, args, target_node_count)
