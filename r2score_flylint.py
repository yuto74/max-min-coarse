...
class MyDataset(object):
    ...
    def _load_all_data(self, training):
        ...
        for n, file in enumerate(glob.glob('data-coarse/*.dot')):
            ...
            coarse_degree = {degree:0 for degree in g.degrees()} # 正解ラベル用
            c_label = []
            for degree in g.degrees():
                coarse_degree[degree] += 1
            for degree in coarse_degree.keys():
                coarse_degree[degree] /= g.nvertices()
            for i in range(1, N_CLASSES + 1):
                try:
                    c_label.append(float(coarse_degree[i]))
                except:
                    c_label.append(.0)
...
def test_model(model, testset):
    ...
    if degree_dist:
        ...
        print('name: only one true')
        label_vec = labels[0]
        for n, true in enumerate(label_vec.detach().cpu().numpy()):
            print(n + 1, true)

        print('name: only one GNN')
        logit_vec = logits[0]
        for n, true in enumerate(logit_vec.detach().cpu().numpy()):
            print(n + 1, true)
    ...
    if r2:
        ...
        print('name: R2 score')
        for i, (logit_vec, label_vec) in enumerate(zip(logits, labels)):
            print(i + 1, float(r2_score(label_vec.detach().cpu().numpy(), logit_vec.detach().cpu().numpy())))
        compare_r2_score_to_coarsedegree(testset)
            

def compare_r2_score_to_coarsedegree(testset):
    print('name: R2 score (vs coarse d)')
    for i, (coarse_vec, label_vec) in enumerate(zip(testset.c_labels, testset.labels)):
        print(i + 1, float(r2_score(label_vec, coarse_vec)))            
